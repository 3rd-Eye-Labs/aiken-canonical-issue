import { addAssets, credentialToAddress, Data, Emulator, EmulatorAccount, generateEmulatorAccount, Lucid, LucidEvolution, TxBuilder, validatorToScriptHash } from '@lucid-evolution/lucid';
import { beforeEach, expect, it } from 'vitest';
import { CanonicalDatum, mkValidator, serialiseCanonicalDatum } from '../src/index.js';
...
it('Non-canonical input vs. canonical output', async (context: LucidContext) => {
  ...
  await runAndAwaitTxBuilder(
    lucid,
    lucid.newTx().pay.ToContract(scriptAddress, { kind: 'inline', value: datumNonCanonical }, { 'lovelace': 5_000_000n }),
  );
  ...
  await runAndAwaitTxBuilder(
    ...
    lucid.newTx()
      ...
      .pay.ToContract(scriptAddress, { kind: 'inline', value: datumCanonical }, { 'lovelace': 5_000_000n }),
  );
});
